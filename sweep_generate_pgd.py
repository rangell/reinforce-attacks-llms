...
singularity exec --nv\
            ...
            -c "source /ext3/env.sh; python run_pgd.py with attack.num_steps=3500 overwrite=__behavior_str__ data.indices=__behavior_idxs__ data.behaviors_path='data/behavior_datasets/strong_reject_behaviors.csv' attack.target_model.model_name_or_path='__model_name__' __use_prefix_cache__"\
"""
...
if __name__ == "__main__":
    ...
    behavior_indices = chunk_list(list(range(100)), 25)


    output_dir = "/scratch/rca9780/jailbreak_analysis_data/reinforce-pgd-logs/"
    ...
    for model_name in models:
        ...
        for behavior_idxs in behavior_indices:
            num_gpus = "1"

            job_name = "{}".format(target_model_id + f"-{min(behavior_idxs)}-{max(behavior_idxs)}")
            out_path = "{}/{}".format(output_dir, target_model_id + f"-{min(behavior_idxs)}-{max(behavior_idxs)}")
            ...
            sbatch_str = sbatch_str.replace("__behavior_str__", f"{min(behavior_idxs)}-{max(behavior_idxs)}")
            sbatch_str = sbatch_str.replace("__behavior_idxs__", str(behavior_idxs).replace(" ", ""))
